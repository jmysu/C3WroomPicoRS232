...
void buttonExample(Control* sender, int type, void* param)
{
    ...
    case B_DOWN:
        ...
        ESPUI.print(statusLabelId, "Down");
        break;

    case B_UP:
        Serial.println("Status: Stop");
        ESPUI.print(statusLabelId, "Up");
        break;
    }
}
...
*//*
    ...
    ESPUI.text("text input", &textCall, ControlColor::Alizarin, "0806449\nABCDEG\n1234");
...
*/
    ...
    sprintf(cDashName, sOTAname.c_str(), (uint16_t)ChipID32);    //for 4 nibbles 
    DashboardId = ESPUI.label((const char*)cDashName, ControlColor::None, sLogo );
    ESPUI.setElementStyle(DashboardId, "font-size: 3em; color: lime;"); //Add style to DashboardId element
     ...
     */
    ESPUI.beginLITTLEFS("C3mBus"); //port80
}
...
void loopESPUI(void)
{
    ...
    {      
        sNodes="";
        if (getNodes()) {
            ESPUI.print(DashboardId, sLogo+ String(getNodes())); //print number of EspNow nodes
 
            for (int i=0;i<getNodes();i++) {
                char buf[32];
                sprintf(buf, "%04X:%02d ", (uint16_t)getNodesMac(i), getNodesRSSI(i));
                sNodes += String(buf);
                }
            //Serial.printf("\nNodes:%s", sNodes.c_str());
            ESPUI.print(NodesId, sNodes);
            }
